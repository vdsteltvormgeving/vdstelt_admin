                        ...
                        <tr>                            
                            ...
                            <th>Bekijken</th>
                                ...
                                if (isset($_POST["sortcat"])) { // Elke if en elseif die hier volgen zijn verschillende clausules voor omhoog en omlaag gesorteerde categorien.
                                    $stmt4 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ORDER BY Category");
                                    ...
                                    while (mysqli_stmt_fetch($stmt4)) 
                                    {
                                        ...
                                        print("<tr><td>$category</td><td>$creation</td><td>$completed</td><td><form><input type='checkbox'</form></td><td><form><input type='checkbox'></form></td><td><form method='POST' action=ticket.php><input type='submit' name='ticket_ID[$ticket_ID]' value='Bekijken'></form></td></tr>");
                                    }
                                } 
                                elseif (isset($_POST["sortcatDESC"])) 
                                {
                                    $stmt5 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ORDER BY Category DESC ");
                                ...
                                {
                                    $stmt6 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ORDER BY creation_date ");
                                ...
                                {
                                    $stmt7 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ORDER BY creation_date DESC ");
                                ...
                                {
                                    $stmt8 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ORDER BY completed_status ");
                                ...
                                {
                                    $stmt9 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ORDER BY completed_status DESC ");
                                ...
                                {
                                    $stmt10 = mysqli_prepare($link, " SELECT category, creation_date, completed_status, ticket_ID FROM ticket T JOIN user U ON t.user_ID = U.user_ID JOIN Customer C On u.user_ID=c.customer_ID ");
